import discord, random, math, time, aiohttp, os
...
class Character():
    ...
    async def ask_age(self):
        ...
        def check(message):
            ...
            try:
                ...
                return message.author == self.ctx.author and message.channel == self.ctx.channel and isinstance(messageContent, int) and messageContent < 30
            except:
                return False
    ...
    async def ask_race(self):
        i = 0
        raceBuffs = []
        embed = await createRaceEmbed()
        msg = await self.ctx.send(embed=embed)
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{url}/races") as response:
                races = await response.json()
        for race in races:
            await msg.add_reaction(race["emoji"])

        def check(reaction, user):
            return user.id == self.ctx.author.id and reaction.message.channel == self.ctx.message.channel and reaction.emoji in [race["emoji"] for race in races]
        
        reaction, user = await self.client.wait_for("reaction_add", check=check, timeout=10800)
    ...
    async def ask_class(self):
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{url}/classes") as response:
                classes = await response.json() 
                embed = await createClassEmbed()
                view = discord.ui.View(timeout=10800)
                
            async def on_click(interaction: discord.Interaction):
                classe = interaction.data["custom_id"].title()
                self.set_class(classe)
                async with session.get(f"{url}/classes/{classe.lower()}") as response:
                    classe = await response.json()
                    self.buffs["agilidadeBuff"] += classe["agiBuff"]
                    self.buffs["forcaBuff"] += classe["forBuff"]
                    self.buffs["inteligenciaBuff"] += classe["intBuff"]
                    self.buffs["presencaBuff"] += classe["preBuff"]
                    self.buffs["vigorBuff"] += classe["vigBuff"]
                    self.buffs["periciaBuff"] += classe["perBuff"]
                    await msg.delete()
    ...
    async def ask_skills(self):
        ...
        async def escolherPericia(interaction: discord.Interaction):
            ...
            if(self.skillsTaken < periciasTotal):
                ...
                for skill in skills:
                    skillName = skill["name"].title()
                    skillDesc = skill["desc"].capitalize()
        ...
        for skill in skills:
            skillName = skill["name"].title()
            skillDesk = skill["desc"].capitalize()
    ...
    async def save(self):
        ...
        async with aiohttp.ClientSession() as session:
            await session.post(f"{url}/characters", json=character, headers=header)
...
async def createClassEmbed():
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(f"{url}/classes") as response:
            classes = await response.json()  
    for classe in classes:
        className = classe["name"].title()
        classDesc = classe["desc"].capitalize()
