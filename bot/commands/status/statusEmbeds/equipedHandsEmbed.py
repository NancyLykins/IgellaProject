...
from Commands.sql.character.select.equip.takeHandItens import takeHandItens
from Commands.sql.character.remove.equip.removeHandEquip import removeHandEquip
...
async def unequipItem(interaction: discord.Interaction):
    ...
    item = takeItem(id)
    effectBuff = {}
    removeHandEquip(interaction.user.id, id, item[7])
...
def equipedHandsEmbed(playerId):
    if(takeHandsEquip(playerId) != []):
        embed = discord.Embed(
            title="Qual item quer desequipar?",
            colour = 128743
        )
        view = discord.ui.View(timeout=180)
        for item in takeHandItens(playerId):
            itemId = item[0]
